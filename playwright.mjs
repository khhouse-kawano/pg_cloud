...
async function fillForm(page, rowValue) {
    ...
    if (rowValue.mobile) await page.fill('#customer_customer_contacts_attributes_0_mobile_phone_number', String(rowValue.mobile)); // 携帯電話
    if (rowValue.landline) await page.fill('#customer_customer_contacts_attributes_0_phone_number', String(rowValue.landline)); // 固定電話
    if (rowValue.mail) await page.fill('#customer_customer_contacts_attributes_0_email', String(rowValue.mail)); // Eメール
    ...
    if (rowValue.zip) {
        await page.fill('#customer_postal_code', String(rowValue.zip)); // 郵便番号
    ...
    if (rowValue.street) await page.fill('#customer_address_detail', String(rowValue.street)); // 番地
    if (rowValue.building) await page.fill('#customer_address_building', String(rowValue.building)); // 建物
...
async function saveData(page, rowValue, retryCount = 0) {
    if (retryCount >= 3) { // 再試行の回数制限を設定（例：3回）
        console.error('Failed to select shop after multiple attempts');
        return;
    }
    ...
    await page.fill('//html/body/main/div[1]/div[3]/form/div[2]/div[1]/div[11]/div[1]/input', String(rowValue.shop)); // 店舗名を入力
    await page.waitForSelector(`div[data-label="${rowValue.shop}"]`); // element
    ...
    if (rowValue.shop) {
        ...
        if (selectedShop === "") {
            await page.fill('//html/body/main/div[1]/div[3]/form/div[2]/div[1]/div[11]/div[1]/input', String(rowValue.shop));
            await page.click(`div[data-label="${rowValue.shop}"]`);
        }
    }
    if (rowValue.date) {
    ...
    if (selectedShopRetry === "" || formattedDateRetry ==="" ) {
        console.error('Unable to select shop. Retrying...');
        await saveData(page, rowValue, retryCount + 1); // 再試行
        return;
    }

    await page.click('//html/body/main/div[1]/div[3]/form/div[4]/div[2]/div[2]/div[1]/button'); // 保存ボタン
    await page.waitForTimeout(1000); // 
    console.log(formattedDateRetry,selectedShopRetry,`${rowValue.firstName}${rowValue.lastName}様のデータを保存しました`);
}  
  // 関数の呼び出し
