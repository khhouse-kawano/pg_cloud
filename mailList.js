...
(async () => {
  ...
  // 複数のページにわたって並行処理
    ...
    const pageElements = await Promise.all([...Array(50).keys()].map(async (i) => {
        ...
        if (match) {
            return { name: name.trim(), id: match[1], date: formattedDate, rank: rank.trim(), shop: shop.trim(), staff: staff.trim(), register: register.trim() };
            }
        }
    return null;
    }));
  ...
  // CSVファイルにデータを書き込む
    const csvContent = 'name,id,register,rank,shop,staff\n' + allElements.map(e => `${e.name},${e.id},${e.register},${e.rank},${e.shop},${e.staff}`).join('\n');
    fs.writeFileSync('mailList.csv', csvContent);
